...
impl RocksDB {
    ...
    /// Copy the current database state to a new directory.
    /// This is a fallback for platforms (like Android) where hard links are not allowed.
    fn copy_snapshot_to_path(&self, dest: &Path) -> Result<(), StoreError> {
        use std::io;

        // Flush WAL to ensure all data is in SST files
        self.db.flush().map_err(|e| StoreError::Internal(e.into()))?;

        // Create destination directory
        fs::create_dir_all(dest).map_err(|e| {
            StoreError::Internal(format!("Failed to create snapshot directory: {}", e).into())
        })?;

        // Copy all files from live directory
        let live_dir = self.dir.join(DIR_LIVE_DB);
        ...
        copy_dir_recursive(&live_dir, dest).map_err(|e| {
            StoreError::Internal(format!("Failed to copy snapshot files: {}", e).into())
        })?;

        tracing::info!(
            target: EVENT_TARGET,
            dest = %dest.display(),
            "Snapshot created via file copy (Android fallback)"
        );
...
impl Store for RocksDB {
    ...
    fn next_snapshot(&'_ self, epoch: Epoch) -> Result<(), StoreError> {
        ...
        // Try checkpoint first (uses hard links for efficiency)
        let checkpoint_result = checkpoint::Checkpoint::new(&self.db)
        ...
        match checkpoint_result {
            ...
            Err(err) => {
                ...
                // On Android, SELinux blocks hard links with "Permission denied"
                // Fall back to file copy in this case
                if err_str.contains("Permission denied") || err_str.contains("link file") {
                    tracing::warn!(
                        target: EVENT_TARGET,
                        "Checkpoint failed (likely Android hard link restriction), falling back to file copy: {}",
                        err_str
                    );
                    self.copy_snapshot_to_path(&path)
                } else {
                    Err(StoreError::Internal(err.into()))
                }
            }
        }
    }
